...
class BLEDeviceConnection: NSObject, DeviceConnectionDelegate {
    @objc weak var delegate: PairUnPairDeviceDelegate?
    override init() {
        super.init()
    }
    // Connect Scanned device.
    @objc func connectScannedDevice(peripheral: CBPeripheral, options: [String : Any]?) {
    ...
    @objc func disConnectScannedDevice(peripheral: CBPeripheral) {
        BLEManager.getSharedBLEManager().connectionDelegate = self
    ...
    @objc func bleManagerConnectionFail(_ central: CBCentralManager, didFailToConnect peripheral: CBPeripheral, error: Error?) {
        delegate?.devicePairedFailed!(peripheral: peripheral, error: error)
    }
    // This method will be triggered once device will be connected.
    @objc func bleManagerDidConnect(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        delegate?.devicePairedSuccessfully!(peripheral: peripheral)
    }
    // This method will be triggered once device will be disconnected.
    @objc func bleManagerDisConect(_ central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
